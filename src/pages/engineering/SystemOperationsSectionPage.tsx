...
import { systemOperationsSectionsData } from '@/data/moet/systemOperationsData';

const SystemOperationsSectionPage = () => {
  const { sectionId = "" } = useParams<{ sectionId: string }>();
  const navigate = useNavigate();
  
  // Get the section data based on the section ID
  const section = systemOperationsSectionsData[sectionId];
  
  // Get all section IDs for navigation
  const sectionKeys = Object.keys(systemOperationsSectionsData).sort();
...
export default SystemOperationsSectionPage;
