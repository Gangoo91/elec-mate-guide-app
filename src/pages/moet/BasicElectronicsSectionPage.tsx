...
import { basicElectronicsSectionsData } from '@/data/moet/basicElectronicsData';

const BasicElectronicsSectionPage = () => {
  const { sectionId = "" } = useParams<{ sectionId: string }>();
  const navigate = useNavigate();
  
  // Get the section data based on the section ID
  const section = basicElectronicsSectionsData[sectionId];
  
  // Get all section IDs for navigation
  const sectionKeys = Object.keys(basicElectronicsSectionsData).sort();
...
export default BasicElectronicsSectionPage;
