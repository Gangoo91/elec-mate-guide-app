...
const Unit203SectionPage = () => {
  const { sectionId = "" } = useParams<{ sectionId: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  
  const sectionKeys = Object.keys(sections203).sort();
  ...
  const sectionContent = sections203[sectionId];
  const isLastSection = sectionId === "1.2";
  
  const handleBackClick = () => {
    navigate('/apprentices/study-materials/city-guilds/level-2/core-units/203');
  };
          ...
          {isLastSection && (
            <div className="mt-8 pt-8 border-t border-[#FFC900]/20">
              ...
              <p className="text-[#FFC900]/80 mb-6">
                Now that you've completed all sections of Unit 203 on Electrical Installations Technology, 
                take this quiz to test your understanding of key concepts.
              </p>
              <SafetyQuiz unitId="203" timeLimit={300} />
            </div>
          )}
        </div>
      </div>
...
export default Unit203SectionPage;
