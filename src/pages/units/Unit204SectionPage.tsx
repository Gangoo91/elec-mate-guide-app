...
const Unit204SectionPage = () => {
  const { sectionId = "" } = useParams<{ sectionId: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  
  const sectionKeys = Object.keys(sections204).sort();
          ...
          {isLastSection && (
            <div className="mt-8 pt-8 border-t border-[#FFC900]/20">
              ...
              <p className="text-[#FFC900]/80 mb-6">
                Now that you've completed all sections of Unit 204 on Installation of Wiring Systems and Enclosures, 
                take this quiz to test your understanding of key concepts.
              </p>
              <SafetyQuiz unitId="204" timeLimit={300} />
            </div>
          )}
        </div>
      </div>
...
export default Unit204SectionPage;
