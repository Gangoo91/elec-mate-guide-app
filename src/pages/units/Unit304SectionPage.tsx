...
const Unit304SectionPage = () => {
  const { sectionId = "" } = useParams<{ sectionId: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  
  const sectionKeys = Object.keys(sections304).sort();
  ...
  const sectionContent = sections304[sectionId];
          ...
          {isLastSection && (
            <div className="mt-8 pt-8 border-t border-[#FFC900]/20">
              ...
              <p className="text-[#FFC900]/80 mb-6">
                Now that you've completed all sections of Unit 304 on Inspection, Testing and Commissioning, 
                take this quiz to test your understanding of key concepts.
              </p>
              <SafetyQuiz unitId="304" timeLimit={300} />
            </div>
          )}
        </div>
      </div>
...
export default Unit304SectionPage;
